...
public class CommandService {
  ...
  // Singleton par instance de View
  ...
  private final String nodeId = UUID.randomUUID().toString(); // identifiant de ce process
  ...
  // paramètres de polling lors d'une reprise
  private static final int POLL_INTERVAL_SEC = 10;
  private static final int POLL_MAX_MINUTES  = 15; // borne max de reprise d’attente
  ...
  private CommandService(ViewContext ctx) {
    ...
    resumeInFlight(); // <-- relance automatique au démarrage
  }

  // ---------------- Public API ----------------

  /** Soumission d’un déploiement/upgrade Helm (asynchrone persistant). */
  public String submitHelmDeploy(HelmDeployRequest req, String repoId, String version, String kubeContext) {
    ...
    e.setPercent(0);
    e.setStep(0);
    e.setMessage("En file d'attente…");
    e.setCreatedAt(now);
    e.setUpdatedAt(now);
  ...
  // ---------------- Relance au démarrage ----------------

  /** Re-enfile toutes les commandes PENDING/RUNNING de cette instance de View. */
  ...
  private void runDeploy(String id, boolean resume) {
    ...
    try {
      tick(e, CommandState.RUNNING, resume ? "Reprise / réconciliation…" : "Validation…", 0, 5);
      ...
      if (resume) {
        ...
        if (current != null) {
          ...
          if (isDeployed(st)) {
            ...
            e.setMessage("Attente de la stabilisation de la release (reprise) …");
            e.setStep(1); e.setPercent(20); e.setUpdatedAt(Instant.now().toString());
            store(e);
            schedulePolling(e.getId(), ns, name, kc);
            return;
          } else if (isFailed(st)) {
            LOG.info("Resuming {}: status FAILED → tentative de redeploiement", name);
          }
        }
      }

      // Deploy / Upgrade
      tick(e, CommandState.RUNNING, "Déploiement Helm…", 1, 30);
      HelmService svc = new HelmService(ctx);
      Release rel = svc.deployOrUpgrade(req, kc, repoId, version);

      tick(e, CommandState.RUNNING, "Finalisation…", 2, 90);
  ...
  /** Planifie un polling régulier (toutes les 10s) jusqu’à stabilité. */
  private void schedulePolling(String id, String ns, String name, String kubeconfig) {
    ...
    final Runnable task = () -> {
      ...
      try {
        ...
        if (r == null) {
          tick(e, CommandState.RUNNING, "Release introuvable, nouvelle tentative de vérification…", e.getStep(), e.getPercent());
          return;
        }
        ...
        if (isDeployed(st)) {
          ...
          fail(e, new IllegalStateException("Release en échec : " + st));
          throw new CancellationException("done");
        } else if (isPending(st)) {
          // rester en attente, maj légère
          e.setMessage("En attente… statut: " + st);
          e.setUpdatedAt(Instant.now().toString());
          e.setPercent(Math.min(85, nvl(e.getPercent()) + 1));
          store(e);
        } else {
          // statut inconnu : continuer à poller
          e.setMessage("Statut courant: " + st);
          e.setUpdatedAt(Instant.now().toString());
          store(e);
        }

        if (System.currentTimeMillis() > deadline) {
          fail(e, new TimeoutException("Délai de reprise dépassé"));
    ...
    // Cancelle proprement quand terminé (CancellationException levée depuis task)
