...
def hbase_decommission(env):
  ...
  if params.hbase_drain_only:
    for host in hosts:
      if host:
        regiondrainer_cmd = format(
          "{kinit_cmd} HBASE_OPTS=\"$HBASE_OPTS {master_security_config}\" {hbase_cmd} --config {hbase_conf_dir} org.jruby.Main {region_drainer2} remove {host}")
    ...
    for host in hosts:
      if host:
        regiondrainer_cmd = format(
          "{kinit_cmd} HBASE_OPTS=\"$HBASE_OPTS {master_security_config}\" {hbase_cmd} --config {hbase_conf_dir} org.jruby.Main {region_drainer2} add {host}")
        regionmover_cmd = format(
          "{kinit_cmd} HBASE_OPTS=\"$HBASE_OPTS {master_security_config}\" {hbase_cmd} --config {hbase_conf_dir} org.jruby.Main {region_mover} -m 24 -o unload -r {host}")
