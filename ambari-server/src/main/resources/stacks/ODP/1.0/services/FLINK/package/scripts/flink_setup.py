  ...
  if action == 'config':
    ...
    if type == 'rest':
      ...
      if params.security_enabled:
        if params.stack_version_formatted and check_stack_feature(StackFeature.FLINK_CONFIG_YAML_SUPPORT, params.stack_version_formatted):
      ...
      if params.stack_version_formatted and check_stack_feature(StackFeature.FLINK_CONFIG_YAML_SUPPORT, params.stack_version_formatted):
        yaml_file_path = format("{params.flink_restserver_conf}/config.yaml")
        header_comment = "# Generated by Apache Ambari. {date}\n".format(
          date=datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")
        )
        yaml_content = yaml.dump(props, default_flow_style=False, allow_unicode=True)
        File(
          yaml_file_path,
          content=header_comment + yaml_content,
          owner=params.flink_user,
    ...
    # Configuring HistoryServer
      ...
      if params.stack_version_formatted and check_stack_feature(StackFeature.FLINK_CONFIG_YAML_SUPPORT, params.stack_version_formatted):
        yaml_file_path = format("{params.flink_historyserver_conf}/config.yaml")
        header_comment = "# Generated by Apache Ambari. {date}\n".format(
          date=datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")
        )
        yaml_content = yaml.dump(props, default_flow_style=False, allow_unicode=True)
      ...
      if params.stack_version_formatted and check_stack_feature(StackFeature.FLINK_CONFIG_YAML_SUPPORT, params.stack_version_formatted):
        yaml_file_path = format("{params.flink_conf}/config.yaml")
        header_comment = "# Generated by Apache Ambari. {date}\n".format(
          date=datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")
        )
        yaml_content = yaml.dump(params.flink_client_properties, default_flow_style=False, allow_unicode=True)
        File(
          yaml_file_path,
          content=header_comment + yaml_content,
          owner=params.flink_user,
