...
class AMBARI_METRICSRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendAmsConfigurationsFromHDP206(self, configurations, clusterData, services, hosts):
    ...
    putAmsSiteProperty("timeline.metrics.cache.size", max(100, int(log(max(total_sinks_count, 1))) * 100))
    putAmsSiteProperty("timeline.metrics.cache.commit.interval", min(10, max(12 - int(log(max(total_sinks_count, 1))), 2)))
