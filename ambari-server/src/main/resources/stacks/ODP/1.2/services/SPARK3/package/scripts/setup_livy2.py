...
def setup_livy(env, type, upgrade_type = None, action = None):
  ...
  PropertiesFile(format("{livy2_conf}/livy-client.conf"),
                properties = params.config['configurations']['spark3-livy2-client-conf'],
                key_value_delimiter = " ",
                owner=params.livy2_user,
                group=params.livy2_group,
  )
...
def setup_symlink(stack_conf_dir = "/usr/odp/current/spark3-livy2/conf", src_livy2_conf_dir = "/etc/spark3-livy2/conf"):
  import params
