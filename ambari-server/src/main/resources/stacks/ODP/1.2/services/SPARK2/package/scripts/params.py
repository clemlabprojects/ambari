...
SERVER_ROLE_DIRECTORY_MAP = {
  ...
  'SPARK2_LIVY2_SERVER' : 'spark2-livy2-server',
  'SPARK2_LIVY2_CLIENT' : 'spark2-livy2-client'
...
if stack_version_formatted and check_stack_feature(StackFeature.SPARK_LIVY2, stack_version_formatted) and "spark2-livy2-env" in config['configurations']:
  livy2_component_directory = Script.get_component_from_role(SERVER_ROLE_DIRECTORY_MAP, "SPARK2_LIVY2_SERVER")
  ...
  livy2_recovery_dir = default("/configurations/spark2-livy2-conf/livy.server.recovery.state-store.url", "/livy2-recovery")
  livy2_recovery_store = default("/configurations/spark2-livy2-conf/livy.server.recovery.state-store", "filesystem")
  ...
  livy2_env_sh = config['configurations']['spark2-livy2-env']['content']
  livy2_log4j_properties = config['configurations']['spark2-livy2-log4j-properties']['content']
  livy2_spark_blacklist_properties = config['configurations']['spark2-livy2-spark-blacklist']['content']

  if 'livy.server.kerberos.keytab' in config['configurations']['spark2-livy2-conf']:
    livy_kerberos_keytab =  config['configurations']['spark2-livy2-conf']['livy.server.kerberos.keytab']
  else:
    livy_kerberos_keytab =  config['configurations']['spark2-livy2-conf']['livy.server.launch.kerberos.keytab']
  if 'livy.server.kerberos.principal' in config['configurations']['spark2-livy2-conf']:
    livy_kerberos_principal = config['configurations']['spark2-livy2-conf']['livy.server.kerberos.principal']
  else:
    livy_kerberos_principal = config['configurations']['spark2-livy2-conf']['livy.server.launch.kerberos.principal']

  livy2_livyserver_hosts = default("/clusterHostInfo/livy2_server_hosts", [])
  livy2_http_scheme = 'https' if 'livy.keystore' in config['configurations']['spark2-livy2-conf'] else 'http'
  ...
  livy2_livyserver_port = default('configurations/spark2-livy2-conf/livy.server.port',8999)
