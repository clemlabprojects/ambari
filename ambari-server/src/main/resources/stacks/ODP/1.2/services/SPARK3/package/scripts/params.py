...
SERVER_ROLE_DIRECTORY_MAP = {
  ...
  'SPARK3_LIVY2_SERVER' : 'spark3-livy2-server',
  'SPARK3_LIVY2_CLIENT' : 'spark3-livy2-client'
...
# spark.sql.hive.metastore.jars=builtin. The bundled jar filename carries
# the full ODP build version (e.g. hive-common-4.2.0.1.3.2.0-14.jar), so
# we derive metastore.version from whichever jar is actually installed —
# making the stack default agnostic to the running ODP build.
hive_metastore_version = "4.2.0"  # safe fallback if the glob misses
try:
...
if stack_version_formatted and check_stack_feature(StackFeature.SPARK_LIVY2, stack_version_formatted) and "spark3-livy2-env" in config['configurations']:
  ...
  livy2_env_sh = config['configurations']['spark3-livy2-env']['content']
