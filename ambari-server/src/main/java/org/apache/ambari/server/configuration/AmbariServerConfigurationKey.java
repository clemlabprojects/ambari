...
public enum AmbariServerConfigurationKey {
  ...
  SSO_JWT_USERNAME_CLAIM(AmbariServerConfigurationCategory.SSO_CONFIGURATION, "ambari.sso.jwt.usernameClaim", PLAINTEXT, "", "The JWT claim used as the Ambari username. When empty, 'preferred_username' is tried first and 'sub' is used as fallback (Keycloak default). Set to 'sub' to restore Knox SSO behavior.", false),
  ...
  SSO_OIDC_PROVIDER_URL(AmbariServerConfigurationCategory.SSO_CONFIGURATION, "ambari.sso.oidc.providerUrl", PLAINTEXT, "", "The Keycloak authorization endpoint URL for the OIDC browser flow (e.g. https://keycloak:8444/realms/my-realm/protocol/openid-connect/auth).  When set, takes precedence over ambari.sso.provider.url for OIDC redirect and token-exchange, allowing Knox SSO (provider.url) and OIDC SSO (oidc.providerUrl) to coexist independently.", false),
  SSO_OIDC_PROVIDER_CERTIFICATE(AmbariServerConfigurationCategory.SSO_CONFIGURATION, "ambari.sso.oidc.providerCertificate", PLAINTEXT, "", "The x509 certificate of the Keycloak realm used to verify OIDC-issued JWT signatures.  When set, tokens signed by either this certificate or ambari.sso.provider.certificate are accepted, enabling mixed Knox-SSO / OIDC-SSO deployments.", false),
  SSO_OIDC_AUTHENTICATION_ENABLED(AmbariServerConfigurationCategory.SSO_CONFIGURATION, "ambari.sso.oidc.authentication.enabled", PLAINTEXT, "", "Whether the OIDC browser-flow authentication is active for Ambari login.  When empty, defaults to true if both oidc.clientId and oidc.clientSecret are set (backward compatibility).  Set explicitly to 'false' to disable the OIDC browser flow while preserving OIDC client credentials for service dispatch.", false),
