...
from resource_management.libraries.functions.ozone_utils import is_https_enabled_in_ozone


__all__ = ["get_om_states", "get_active_om",
           "get_property_for_active_oms", "get_property_for_active_om", "get_ozone_om_nameservices",
           "get_name_service_by_hostname"]

OZONE_OM_STATE_ACTIVE = 'active'
OZONE_OM_STATE_STANDBY = 'standby'

OM_HTTP_NON_HA = 'ozone.om.http-address'
OM_HTTPS_NON_HA = 'ozone.om.https-address'
HDDS_HTTP_POLICY = "ozone.http.policy"

OM_HTTP_FRAGMENT = 'ozone.om.http-address.{0}.{1}'
OM_HTTPS_FRAGMENT = 'ozone.om.https-address.{0}.{1}'
OM_RPC_FRAGMENT = 'ozone.om.address.{0}.{1}'
OM_RPC_NON_HA = 'ozone.om.address'
...
def get_leader_om(ozone_site, security_enabled, run_user, times=5, sleep_time=1, backoff_factor=2, name_service=None):
  """
  return format [('om1', 'hostname1:port1'), ('om2', hostname2:port2')] , [....], [....]
  """
...
def  get_namenode_states_noretries(hdfs_site, security_enabled, run_user, last_retry=True, name_service=None):
  ...
  name_services = get_ozone_om_nameservices(hdfs_site) if not name_service else [name_service]
...
def all_jmx_namenode_addresses(hdfs_site, name_service):
  ...
  for nn_unique_id in nn_unique_ids:
    is_https_enabled = is_https_enabled_in_ozone(hdfs_site['dfs.http.policy'])

    rpc_key = OM_RPC_FRAGMENT.format(name_service,nn_unique_id)
    if not is_https_enabled:
      key = OM_HTTP_FRAGMENT.format(name_service,nn_unique_id)
      protocol = "http"
    else:
      key = OM_HTTPS_FRAGMENT.format(name_service,nn_unique_id)
      protocol = "https"
...
def _is_om_ha_config(hdfs_site):
  """
  returns True if an HA config is used
  """
  name_services = hdfs_site.get('ozone.om.service.ids', None)
  if name_services:
    for ns in name_services.split(","):
      if hdfs_site.get('ozone.om.nodes.'+ns):
        return True
  return False
...
def get_active_namenode(hdfs_site, security_enabled, run_user):
  ...
  """
  active_namenodes = get_leader_om(hdfs_site, security_enabled, run_user)[0]
  if active_namenodes:
...
def get_property_for_active_namenodes(hdfs_site, property_name, security_enabled, run_user):
  ...
  name_services = get_ozone_om_nameservices(hdfs_site)
  for name_service in name_services:
...
def get_property_for_active_namenode(hdfs_site, name_service, property_name, security_enabled, run_user):
  ...
  For dfs.namenode.rpc-address:
    - In non-ha mode it will return ozone_site[dfs.namenode.rpc-address]
    - In ha-mode it will return ozone_site[dfs.namenode.rpc-address.nnha.nn2], where nnha is the name of HA, and nn2 is id of active NN
  """
  value = None
  rpc_key = None
  if _is_om_ha_config(hdfs_site):
    name_services = get_ozone_om_nameservices(hdfs_site)
    ...
    active_namenodes = get_leader_om(hdfs_site, security_enabled, run_user, name_service=name_service)[0]
    ...
    rpc_key = OM_RPC_FRAGMENT.format(name_service,active_namenode_id)
  else:
...
def get_ozone_om_nameservices(ozone_site):
  """
  ...
  list in ozone_site['ozone.om.service.ids']. The parameter
  hdfs['ozone.om.internal.service.id'] was introduced in Ozone 1.2.1 to denote the
  nameservice for the current cluster.
  In federated mode multiple name services will be returned.

  This method uses ozone_site['ozone.om.internal.service.id'] to get the current
  nameservice(s), if that parameter is not available it tries to splits the value
  in ozone_site['ozone.om.service.ids'] returning the first string or what is
  contained in ozone['ozone.om.nodes'].

  By default ozone_site['ozone.om.service.ids'] is returned.
  :param ozone_site:
  :return: list of string or an empty list
  """

  name_services_param = ozone_site.get('ozone.om.internal.service.id', None)
  if name_services_param:
    name_services = name_services_param.split(",")
    return name_services

  name_services_string = ozone_site.get('ozone.om.service.ids', None)

  if name_services_string:
    import re
    for ns in name_services_string.split(","):
      if 'ozone.om.nodes' in ozone_site and re.match(r'.*%s$' % ns, ozone_site['ozone.om.nodes']): # better would be ozone_site['fs.defaultFS'] but it's not available
        return [ns]
    return [name_services_string.split(",")[0]] # default to return the first nameservice
  return []


def get_initial_active_om(ozone_env):
  """
  :return: The set of initially active owo hosts as specified by one of the
           'ozone-env/hdds_ha_initial_om_active' properties.
           An empty set if the properties are unspecified or empty.
  """
  setting = ''

  if 'hdds_ha_initial_om_active' in ozone_env and not setting:
    setting = ozone_env['hdds_ha_initial_om_active']
...
def get_namespace_mapping_for_hostname(hostname, hdfs_site, security_enabled, run_user):
  namenode_address_map_list = get_leader_om(hdfs_site, security_enabled, run_user)
  ...
  namenode_nameservices = get_ozone_om_nameservices(hdfs_site)
